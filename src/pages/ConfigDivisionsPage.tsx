...
const ConfigDivisionsPage = () => {
  ...
  const handleSelect = async (id: number) => {
    if (id) {
      try {
        ...
        toast.error("Error getting Division Details");
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleUpdate = async () => {
    if (titleID) {
      try {
        ...
        toast.error("Error Updating Division Details");
      } finally {
        setUpdating(false);
      }
    }
  ...
  const handleDelete = async () => {
    if (titleID) {
      try {
        setUpdating(true);
        const response = await axiosInstance.delete(`/divisions/${titleID}`);
  ...
  return (
    <Layout>
      <main className="wrapper">
        <Header location="Divisions Configuration" />
        <div>
          <BackBTN />
        </div>
        <TitleForm purpose="division" />
          ...
          <div className="w-1/2 flex flex-col items-center justify-start gap-10">
            ...
            {title && (
              <Deletor
                ...
                placeholder="Division Id Must Be Provided"
                question="Provide the Division   ID"
...
export default ConfigDivisionsPage;
