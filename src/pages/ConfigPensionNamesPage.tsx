...
import { BankTypes, TitleTypes } from "../types";
import toast from "react-hot-toast";
import { axiosInstance } from "../libs";
import { getPensionBanks } from "../utils/requests";
...
const ConfigPensionNamesPage = () => {
  ...
  const handleSelect = async (id: number) => {
    if (id) {
      try {
        setLoading(true);
        const response = await axiosInstance.get("/pensionAccount/" + id);
  ...
  const handleUpdate = async () => {
    if (id) {
      try {
        setUpdating(true);
        const response = await axiosInstance.patch(`/pensionAccount/${id}`, {
          id: id,
  ...
  const handleDelete = async () => {
    if (id) {
      try {
        setUpdating(true);
        const response = await axiosInstance.delete(`/pensionAccount/${id}`);
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await getPensionBanks();
      if (response.status == 200) {
  ...
  return (
    <Layout>
      <main className="wrapper">
        ...
        {/* <BankForm purpose="pension bank" /> */}
        <TitleForm purpose="pension bank" />
