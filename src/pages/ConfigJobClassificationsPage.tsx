...
import { getAllJobClassifications } from "../utils/requests";
import TextInputWithCopy from "../components/TextInputWithCopy";

const ConfigJobClassificationsPage = () => {
  ...
  const handleSelect = async (id: number) => {
    if (id) {
      try {
        setLoading(true);
        const response = await axiosInstance.get("/classifications/" + id);
        ...
        toast.error("Error getting Classification Details");
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleUpdate = async () => {
    if (titleID) {
      try {
        ...
        toast.error("Error Updating Classification Details");
      } finally {
        setUpdating(false);
      }
    }
  ...
  const handleDelete = async () => {
    if (titleID) {
      try {
        ...
        toast.error("Error Deleting Classification");
      } finally {
        setUpdating(false);
      }
    }
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await getAllJobClassifications();
      if (response.status == 200) {
  ...
  return (
    <Layout>
      <main className="wrapper">
        <Header location="Jobs Classification Configuration" />
        <div>
          <BackBTN />
        </div>
        <TitleForm purpose="job classification" />
          ...
          <div className="w-1/2 flex flex-col items-center justify-start gap-10">
            ...
            {title && (
              <Deletor
                ...
                placeholder="Classification Id Must Be Provided"
                question="Provide the Classification   ID"
...
export default ConfigJobClassificationsPage;
